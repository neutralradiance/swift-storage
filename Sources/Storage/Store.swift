...
public struct Store<Value: AutoCodable>: DefaultsWrapper {
    ...
    public var wrappedValue: Value? {
        get {
            do {
                guard let data =
                        store.object(forKey: key) as? Value.AutoDecoder.Input else {
                    return nil
                }
    ...
    public init(wrappedValue: Value? = nil, _ key: String, store: UserDefaults? = nil) {
        self.key = key
...
extension Store: AutoCodable {
    public init(from decoder: Decoder) throws {
        ...
        if let data = store.data(forKey: key) as? Value.AutoDecoder.Input {
            self.wrappedValue = try Self.decoder.decode(Value.self, from: data)
        }
    }
