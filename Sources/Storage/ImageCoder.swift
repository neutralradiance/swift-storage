...
public class ImageDecoder: TopLevelDecoder {
    public func decode<T>(_ type: T.Type, from data: Data) throws -> T where T: Decodable {
        guard let object = try JSONDecoder().decode(type, from: data) as? SerializedImage else {
            throw DecodingError.dataCorrupted(
                .init(codingPath: [], debugDescription: "Couldn't read data for decoder."
                )
            )
        }
        return object as! T
    ...
    public func encode<T>(_ value: T) throws -> Data where T: Encodable {
        guard let object = value as? SerializedImage,
            ...
            throw EncodingError.invalidValue(
                value, .init(codingPath: [], debugDescription: "Invalid input for encoder."
                )
            )
        }
        ...
        if let timestamp = object.timestamp {
            json.updateValue(timestamp.timeIntervalSinceReferenceDate, forKey: "timestamp")
        }
        if let expiration = object.expiration {
            json.updateValue(expiration.timeIntervalSinceReferenceDate, forKey: "expiration")
        }
...
public extension ImageEncoder {
    ...
    enum EncodingStrategy {
        case
            ...
            png,
             /// JPEG format.
            /// - parameter compressionQuality: 0 to 1, with zero being the highest level of compression.
             jpeg(_ compressionQuality: CGFloat)
    }

    #if os(iOS)
    static func encode(image: UIImage, using strategy: EncodingStrategy) -> Data? {
        switch strategy {
        case .png:
            guard let data = image.pngData() else { break }
            return data
        case let .jpeg(quality):
            guard let data = image.jpegData(compressionQuality: quality) else { break }
            return data
        }
        return nil
    }
    #elseif os(macOS)
    static func encode(image: NSImage, using strategy: EncodingStrategy) -> Data? {
        switch strategy {
        case .png:
            guard let data = image.pngData() else { break }
            return data
